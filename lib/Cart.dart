...
class StateCart extends State<Cart> with TickerProviderStateMixin {
                                            ...
                                            onBtnSelected: _placeOrder
                                                ? () async {
                                                    ...
                                                    if (selAddress == null ||
                                                      ...
                                                      var deliveryDateTime = await Navigator.push(
                                                          context,
                                                          MaterialPageRoute(
                                                              builder: (BuildContext
                                                                      context) =>
  ...
  int allowDate() {
    baselist = deliveryDateTime1.split(';');
    var Cday = DateTime.now().day;
    var Cmonth = DateTime.now().month;
    var Cyear = DateTime.now().year;
    var Chour = DateTime.now().hour;

    var list1 = baselist[0].split(" ");

    var list2 = baselist[1].split(" ");

    var listHP = list2[1].split(':');
    var listHD = list1[1].split(':');

    var listD = list1[0].split('-');
    var listP = list2[0].split('-');

    if (listD.isEmpty || listP.isEmpty) return 1;

    var yearD = int.parse(listD[0]);
    var monthD = int.parse(listD[1]);
    var dayD = int.parse(listD[2]);

    var yearP = int.parse(listP[0]);
    var monthP = int.parse(listP[1]);
    var dayP = int.parse(listP[2]);

    var hourP = int.parse(listHP[0]);
    if (hourP > 12) hourP = hourP - 12;

    var hourD = int.parse(listHD[0]);
    if (hourD > 12) hourD = hourD - 12;
    ...
    if (Cyear == yearD &&
      ...
      } else if (Chour < hourP && hourP < hourD) {
        allowed = 0;
      }
    }
    return allowed;
    ...
    if (_isNetworkAvail) {
      ...
      try {
        var parameter = {
          ...
          ADD_ID: selAddress,
          ADD_P_ID: addressList[selectedAddressPickup].id,
        ...
        if (isTimeSlot) {
          ...
          parameter[DELIVERY_DATE] = baselist[0];
          parameter[PICKUP_DATE] = baselist[1];
        }
        if (isPromoValid) {
                ...
                : Expanded(
                    child: Padding(
                      ...
                      child: GestureDetector(
                        child: Text(
                          getTranslated(context, 'ADDADDRESS_Delivery'),
                          style: TextStyle(
                              color: colors.fontColor,
                              fontWeight: FontWeight.bold),
                        ),
            ...
            addressList.length > 0
                        ...
                        children: [
                          Padding(
                            padding:
                                const EdgeInsetsDirectional.only(bottom: 5.0),
                            child:
                                Text(addressList[selectedAddressPickup].name),
                          ),
                          Text(
                            addressList[selectedAddressPickup].address +
                                ", " +
                                addressList[selectedAddressPickup].area +
                                ", " +
                                addressList[selectedAddressPickup].city +
                                ", " +
                                addressList[selectedAddressPickup].state +
                                ", " +
                                addressList[selectedAddressPickup].country +
                                ", " +
                                addressList[selectedAddressPickup].pincode,
                            style: Theme.of(context)
                                .textTheme
                                .caption
                                .copyWith(color: colors.lightBlack),
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: [
                                Text(
                                  addressList[selectedAddressPickup].mobile,
                                  style: Theme.of(context)
                                      .textTheme
                                      .caption
                                      .copyWith(color: colors.lightBlack),
                ...
                : Expanded(
                    child: Padding(
                      ...
                      child: GestureDetector(
                        child: Text(
                          getTranslated(context, 'ADDADDRESS_pick_Up'),
                          style: TextStyle(
                              color: colors.fontColor,
                              fontWeight: FontWeight.bold),
                        ),
          ...
          var deliveryDateTime = await Navigator.push(
              ...
              MaterialPageRoute(
                  builder: (BuildContext context) => Payment(
                      ...
                      msg: msg)));
          deliveryDateTime1 = deliveryDateTime;
